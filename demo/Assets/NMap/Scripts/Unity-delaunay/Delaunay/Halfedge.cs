...
namespace ET
{
    public sealed class Halfedge : IDisposable
    {
        ...
        public static Halfedge Create(Edge edge, Nullable<LRSide> lr)
        {
            if (_pool.Count > 0)
            {
        ...
        public Edge edge;
        public Nullable<LRSide> leftRight;
        public Vertex vertex;

        // the vertex's y-coordinate in the transformed Voronoi space V*
        public float ystar;

        public Halfedge(Edge edge = null, Nullable<LRSide> lr = null)
        {
            Init(edge, lr);
        }

        private Halfedge Init(Edge edge, Nullable<LRSide> lr)
        {
            this.edge = edge;
            leftRight = lr;
        ...
        internal bool IsLeftOf(float2 p)
        {
            ...
            if (rightOfSite && this.leftRight == LRSide.LEFT)
            {
                return true;
            }

            if (!rightOfSite && this.leftRight == LRSide.RIGHT)
            {
                return false;
            }
            ...
            return this.leftRight == LRSide.LEFT ? above : !above;
        }
    }
}