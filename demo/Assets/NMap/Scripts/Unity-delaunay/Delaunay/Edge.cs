...
namespace ET
{
    ...
    public sealed class Edge
    {
        ...
        public GeoLineSegment VoronoiEdge()
        {
            ...
            return new GeoLineSegment(_clippedVertices[LRSide.LEFT],
                _clippedVertices[LRSide.RIGHT]);
        }
        ...
        public Vertex Vertex(LRSide leftRight)
        {
            return (leftRight == LRSide.LEFT) ? _leftVertex : _rightVertex;
        }

        public void SetVertex(LRSide leftRight, Vertex v)
        {
            if (leftRight == LRSide.LEFT)
            {
                _leftVertex = v;
            }
            else
        ...
        private Dictionary<LRSide, Nullable<float2>> _clippedVertices;

        public Dictionary<LRSide, Nullable<float2>> clippedEnds
        {
        ...
        private Dictionary<LRSide, Site> _sites;

        public Site leftSite
        {
            get { return _sites[LRSide.LEFT]; }
            set { _sites[LRSide.LEFT] = value; }
        }

        public Site rightSite
        {
            get { return _sites[LRSide.RIGHT]; }
            set { _sites[LRSide.RIGHT] = value; }
        }

        public Site Site(LRSide leftRight)
        {
...
//			}
            ...
            {
                _clippedVertices[LRSide.LEFT] = null;
                _clippedVertices[LRSide.RIGHT] = null;
                _clippedVertices = null;
            }

            _sites[LRSide.LEFT] = null;
            _sites[LRSide.RIGHT] = null;
            _sites = null;
        ...
        private void Init()
        {
            _sites = new Dictionary<LRSide, Site>();
        }

        public override string ToString()
        {
            return "Edge " + _edgeIndex.ToString() + "; sites " + _sites[LRSide.LEFT].ToString() + ", " +
                   _sites[LRSide.RIGHT].ToString()
        ...
        public void ClipVertices(RectangleF bounds)
        {
            ...
            _clippedVertices = new Dictionary<LRSide, Nullable<float2>>();
            if (vertex0 == _leftVertex)
            {
                _clippedVertices[LRSide.LEFT] = new float2(x0, y0);
                _clippedVertices[LRSide.RIGHT] = new float2(x1, y1);
            }
            else
            {
                _clippedVertices[LRSide.RIGHT] = new float2(x0, y0);
                _clippedVertices[LRSide.LEFT] = new float2(x1, y1);
            }
        }
    }
}
